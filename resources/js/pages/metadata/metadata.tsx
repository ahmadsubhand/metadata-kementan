...
export default function Metadata({ metadata_form }: { metadata_form?: Omit<MetadataStoreType, 'id'> & { id: string | number } }) {
    ...
    const form = useForm({
        resolver: zodResolver(metadataStoreSchema), mode: 'onChange', defaultValues: {
            ...
            activity_year: metadata_form?.activity_year || dayjs().year(),
            data_collection_approach_id: metadata_form?.data_collection_approach_id || 0,
            activity_sector_id: metadata_form?.activity_sector_id || 0,
            statistical_activity_type_id: metadata_form?.statistical_activity_type_id || 0,
            statistical_activity_recommendation: metadata_form?.statistical_activity_recommendation || 0,
            ...
            collected_variables: metadata_form?.collected_variables || null,

            // IV. DESAIN KEGIATAN
            activity_conduct_id: metadata_form?.activity_conduct_id || null,
            frequency_of_implementation_id: metadata_form?.frequency_of_implementation_id || null,
            data_collection_type_id: metadata_form?.data_collection_type_id || null,
            data_collection_coverage_id: metadata_form?.data_collection_coverage_id || null,
            data_collection_methods: metadata_form?.data_collection_methods || null,
            data_collection_method_other: metadata_form?.data_collection_method_other || '',
            data_collection_tools: metadata_form?.data_collection_tools || null,
            data_collection_tool_other: metadata_form?.data_collection_tool_other || '',
            data_collection_units: metadata_form?.data_collection_units || null,
            data_collection_unit_other: metadata_form?.data_collection_unit_other || '',
            activity_regions: metadata_form?.activity_regions || null,

            // V. DESAIN SAMPEL
            sample_design_type_id: metadata_form?.sample_design_type_id || null,
            final_stage_sampling_method_id: metadata_form?.final_stage_sampling_method_id || null,
            sampling_method_id: metadata_form?.sampling_method_id || null,
            final_stage_sampling_frame_id: metadata_form?.final_stage_sampling_frame_id || null,
            overall_sample_fraction: metadata_form?.overall_sample_fraction || null,
            estimated_sampling_error: metadata_form?.estimated_sampling_error || null,
            sampling_unit: metadata_form?.sampling_unit || null,
            observation_unit: metadata_form?.observation_unit || null,
            // A component is changing an uncontrolled input to be controlled error still exist on this section

            // VI. PENGUMPULAN DATA
            pilot_survey: metadata_form?.pilot_survey || null,
            data_quality_check_methods: metadata_form?.data_quality_check_methods || null,
            data_quality_check_method_other: metadata_form?.data_quality_check_method_other || null,
            nonresponse_adjustment: metadata_form?.nonresponse_adjustment || null,
            data_collector_type_id: metadata_form?.data_collector_type_id || null,
            minimum_education_requirement_id: metadata_form?.minimum_education_requirement_id || null,
            number_of_supervisors: metadata_form?.number_of_supervisors || null,
            number_of_enumerators: metadata_form?.number_of_enumerators || null,
            training_of_data_collector: metadata_form?.training_of_data_collector || null,

            // VII. PENGOLAHAN DAN ANALISIS
            editing_step: metadata_form?.editing_step || null,
            coding_step: metadata_form?.coding_step || null,
            data_entry_step: metadata_form?.data_entry_step || null,
            validation_step: metadata_form?.validation_step || null,
            analysis_method_id: metadata_form?.analysis_method_id || null,
            analysis_units: metadata_form?.analysis_units || null,
            analysis_unit_other: metadata_form?.analysis_unit_other || '',
            presentation_levels: metadata_form?.presentation_levels || null,
            presentation_level_other: metadata_form?.presentation_level_other || '',

            // VIII. DISEMINASI HASIL
            printed_product: metadata_form?.printed_product || null,
            digital_product: metadata_form?.digital_product || null,
            microdata_product: metadata_form?.microdata_product || null,
    ...
    function clearConditionalQuestion(data: MetadataStoreType) {
        ...
        // Clear checkbox other option
        if (!data.data_collection_methods?.includes(5)) {
            data.data_collection_method_other = null
        }
        if (!data.data_collection_tools?.includes(6)) {
            data.data_collection_tool_other = null;
        }
        if (!data.data_collection_units?.includes(4)) {
            data.data_collection_unit_other = null;
        }
        if (!data.data_quality_check_methods?.includes(4)) {
            data.data_quality_check_method_other = null;
        }
        if (!data.analysis_units?.includes(4)) {
            data.analysis_unit_other = null;
        }
        if (!data.presentation_levels?.includes(5)) {
            data.presentation_level_other = null;
        }
